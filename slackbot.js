...
rtm.on(RTM_EVENTS.MESSAGE, function (message) {
  ...
  .then((response) => {
    if(response.data.result.parameters.invitees && response.data.result.parameters.invitees === 0) {
      rtm.sendMessage('Who are you meeting with?', message.channel);
      return;
    }
    if (response.data.result.parameters.time && response.data.result.parameters.time.length === 0) {
    ...
    if (response.data.result.parameters.task && response.data.result.parameters.task.length === 0) {
